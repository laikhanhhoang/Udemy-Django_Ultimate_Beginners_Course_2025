...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_mail("Welcome to EdenThought!", "Congratulations on creating an account with EdenThought.", settings.DEFAULT_FROM_EMAIL, [current_user.email], fail_silently=True)
...
def my_login(request):
    ...
    return render(request, 'journal/my-login.html',  context = {'LoginForm': form})
...
def create_thought(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            thought.save()
            messages.success(request, 'New thought created successfully !')
            return redirect('my-thoughts')
